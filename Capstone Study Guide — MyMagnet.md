...
| Custom logs empty at first | LIVE, resolved by waiting | The agent configurations were `VALID`/`ACTIVE`, the dynamic group matched both instances, and `use log-content` existed, yet nothing arrived for ~25 minutes. About an hour after apply, 1,263 nginx access-log entries had arrived with no change made. App and nginx-error logs still show none (possibly empty files or a path mismatch; unconfirmed). | A new agent configuration can take a long time to reach instances. Verify the OCI side (config state, dynamic group, `use log-content`), then wait before changing things. |
| Connector Hub rejected an empty source | LIVE, resolved | Creating the connector failed twice with "No log sources found to be read" while the logs were empty. Once nginx access logs arrived, the same connector (`enable_log_connector = true`) was created in 41 s. | Connector Hub needs source data to exist. A Logging source needs no extra policy. An Object Storage target needs the `any-user … request.principal.type='serviceconnector'` policy. Batch rollover is at most 7 minutes. |
