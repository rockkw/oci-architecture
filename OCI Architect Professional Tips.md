...
| Network Firewall rules are evaluated **before NSG rules** (or NSGs first, or NSGs ignored once a firewall is in the path) | Independent layers: NSGs/security lists enforce at each VNIC (including the firewall's own VNIC); the firewall is a routed hop via route tables. Traffic must be allowed by **both**. Don't confuse with the firewall's internal order (row above). Key word is **rules**: physical packet order depends on routing, rule evaluation doesn't. Same model as AWS (SGs, NACLs, AWS Network Firewall). | Att 2 Q33 (missed) | [[5. Security — OCI IAM, WAF, Certificates, Vault, Cloud Guard]] |
...
| `oci kms crypto encrypt` sent to the vault's `…-management.kms…` endpoint fails: "pass the key version OCID", "forgot the region", or "plaintext must be JSON" | **Wrong endpoint**: crypto operations need the `…-crypto.kms…` endpoint. Plaintext is base64; region is already in the OCID and URL. Tip: read the endpoint name in the command first | Att 1 Q8 (picked region); Att 2 Q49 (picked key version OCID) **(missed twice)** | [[5. Security — OCI IAM, WAF, Certificates, Vault, Cloud Guard]] |
...
| OKE app → ATP: env vars with the ATP OCID + OCI API credentials, then call an ATP **"CreateConnection" API** (fabricated, in a reversed "which is NOT valid" question) | No such API; OCI APIs only manage the DB. Valid data paths: SQL*Net/TLS with wallet or connect string (wallet in a K8s Secret mounted as a volume **is valid**), OCI Service Broker binding, or ORDS over HTTPS | Att 2 Q30 (missed: picked the valid wallet option) | [[12. Containers — OCI OKE, Container Instances, OCIR]] |
...
| Base DB version **upgrades are rolling / no downtime on 2-node RAC**; pre-upgrade automatic backups can restore to the earlier version | Upgrades are **not** rolling and need downtime even on RAC (RAC rolls patching and fixed-shape scaling, not upgrades). Pre-upgrade backups **can't** restore to a pre-upgrade point after the upgrade | Att 2 Q40 (missed) | [[6. Databases — OCI Database, NoSQL, Caching, DR]] |
| ATP-S scenario pick: skipping "only used during work hours, keep costs low"; or running E-Business Suite on Autonomous Serverless | Work-hours use fits ATP-S **stop/start** (a stopped DB bills storage only). E-Business Suite isn't supported on Autonomous Serverless (use Base DB/Exadata). No answer key | Att 2 Q36 (likely partly missed) | [[6. Databases — OCI Database, NoSQL, Caching, DR]] |
...
| Same-region VCN-to-VCN routing by **peering the DRG to an LPG** (or enabling BGP from DRG to VCN) | An LPG peers only with another LPG. Valid: LPG↔LPG local peering, or one DRG with both VCNs attached and routes pointing at the DRG (modern hub, preferred) | Att 2 Q45 (half-missed) | [[9. Networking — OCI VCN, DRG, Gateways, Load Balancers]] |
