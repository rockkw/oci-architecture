...
| 2 | SQLite → Autonomous AI Database (26ai) + AI Vector Search | Databases | `lab-basedb-stack` (network pattern), `lab-mymagnet-stack` (Vault, dynamic group) | **Live 2026-09-25.** 9 of 10 resources applied (all but the `adb_secret_read` IAM policy, left for a person). ADB 26ai (23.26.3.3), private endpoint `10.0.2.243`, TLS on 1522. From a SQLcl pod in OKE (`capstone-sql` namespace): scripts 01–04 ran; `MAGNET.ALL_MINILM_L12_V2` loaded (384 dims); 6 tables + HNSW index created. Semantic search verified with rolled-back sample rows ("linux operating system installer" → Debian/Ubuntu/Fedora). Tables empty until the app patch and SQLite migration |
...
| 5 | Observability (UMA logs, custom metrics, alarms → Notifications) and HA/DR (fault domains, cross-region bucket replication, DNS failover) | Observability, HA/DR | Lab 2 design | Applied 2026-09-25: all 19 resources, including Connector Hub (created once logs had data). nginx access logs flowing; alarms OK; email subscription confirmed (`ACTIVE`). HA/DR designed below |
...
| Confirm app and nginx-error logs | **Logs flow (2026-09-25):** about an hour after apply, 1,263 `/var/log/nginx/access.log` entries had arrived, with no manual agent install (the plugin runs on A1). Connector Hub was then created with `enable_log_connector = true` (tfvars) and succeeded, confirming it had failed only because the source was empty. No entries yet from the app log glob or `nginx/error.log`: the error log may just be empty; the app path is unconfirmed. The first Run Command job stayed `ACCEPTED` (likely created before its policy propagated) | Create a **new** Run Command on instance 1: `ls -la /opt/magnetlookup/data/logs /var/log/nginx` and check the paths match the `app` agent config |
...
| Point the Phase 3 function at the Phase 4 model | `llm_endpoint = "http://10.0.2.231"` and `llm_model = "qwen2.5-1.5b-instruct"` are set in the gitignored tfvars (plan checked 2026-09-25). Still needed: the API key as a Vault secret; without it the function gets 401 and records `llm_error` | Store the key: `kubectl -n vllm get secret vllm-api-key -o jsonpath='{.data.api-key}' \| base64 -d`, create a secret in the mymagnet Vault with that value, set `llm_api_key_secret_ocid`, then plan and apply the enrich stack |
| Apply the `adb_secret_read` IAM policy (the one resource not yet applied in `lab-capstone-adb-stack`) | The rest of the stack was applied with `-target` on 2026-09-25; auto mode blocks IAM grants. Only needed when the app patch rolls out (instances read the MAGNET password from Vault) | `cd terraform/lab-capstone-adb-stack && terraform plan -out=adb.tfplan && terraform apply adb.tfplan` (plan should show 1 to add) |
