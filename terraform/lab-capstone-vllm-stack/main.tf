...
# This stack owns only the OCI-side network pieces for the Phase 4 vLLM endpoint.
# The Kubernetes objects (Namespace, PVC, Deployment, Service) are rendered from
# vllm.yaml.tftpl into the `vllm_manifest` output and applied by hand with kubectl,
# the same split lab-oke-app-stack uses (no kubernetes/helm provider in this repo).
#
# Traffic path: client in 10.0.2.0/24 -> internal LB (TCP 80, in 10.0.2.0/24)
# -> GPU worker node NodePort (30000-32767) -> vLLM pod (8000).
...
# The GPU node pool must be in the workers NSG for these to apply (see
# lab-oke-gpu-stack's workers_nsg_id variable).
...
locals {
  vllm_manifest = templatefile("${path.module}/vllm.yaml.tftpl", {
